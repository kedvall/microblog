...
def export_posts(user_id):
    try:
        ...
        for progress, post in enumerate(user.posts.order_by(Post.timestamp.asc())):
            data.append({'body': post.body, 'timestamp': post.timestamp.isoformat() + 'Z'})
            time.sleep(3)  # Makes export take longer so the progress increase can actually be seen...
            _set_task_progress(100 * progress // total_posts)
